...
{
    log.Info("Request for Source List provisioning has been submitted.");
    try 
    {
...
private static async Task Provision(string siteUrl, string listTitle)
{
    using (var ctx = await GetClientContext(siteUrl))
    {
        ...
        if (list == null)
        {
            ...
            list.Fields.AddFieldAsXml(@"<Field ID='{1511BF28-A787-4061-B2E1-71F64CC93FD5}'
                                                    Name='Date'
                                                    DisplayName='Date'
                                                    Type='DateTime'
                                                    Format='DateOnly'
                                            ...
                                            true,
                                            AddFieldOptions.AddFieldToDefaultView | AddFieldOptions.AddFieldInternalNameHint);
            ctx.ExecuteQuery();
        }
    }
}
